...
class _KycState extends State<Kyc> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Align(
                ...
                child: InkWell(
                  onTap: () {
                    askConfirmation(
                      context,
                      "Are you sure exit Kyc ?",
                      confirmCallBack: () {
                        Navigator.pop(context);
                      },
                    );
                  },
                  child: Icon(Icons.close_sharp, color: AppColors.redLight),
                ),
              ),
              ...
              Expanded(
                child: BlocBuilder<KycBloc, KycState>(
                  ...
                  builder: (context, state) {
                    if (state is KycUploadingProgressState) {
                      return Center(
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              10.height(),
                              Text(
                                "This might take a few minutes depending on your internet speed. Please do not close the app.",
              ...
              CustomButton(
                buttonText: screen == 3 ? "Submit" : "Save & Next",
                onPress: () async {
                  hideKeyboard();

                  if (screen == 0) {
                    print(
                      "KycController.checkPhoto.text ${KycController.checkPhoto.text}",
                    );
                    hiveBloc.add(
                      HiveSaveBusinessKycEvent(
                  ...
                  if (screen == 3) {
                    hiveBloc.add(
                      HiveSaveUploadKycEvent(
                        uploadKycModel: UploadKycModel(
                          businessRegistrationProof:
                              KycController.businessProofPhoto.text,
                          gstCertificate:
                              KycController.gstCertificatePhoto.text,
                          panCard: KycController.panCertificatePhoto.text,
                          fssaiLicense:
                              KycController.fssaiCertificatePhoto.text,
                        ),
                      ),
                    );

                    if (!(KycController.businessKycFormKey.currentState
                            ?.validate() ??
                        false)) {
                      print("kjfslfs");
                      setState(() {
                      ...
                      return;
                    } else if (!(KycController.bankKycFormKey.currentState
                            ?.validate() ??
                        false)) {
                      setState(() {
                        screen = 1;
                      ...
                      return;
                    } else if (!(KycController.ownerKycFormKey.currentState
                            ?.validate() ??
                        false)) {
                      setState(() {
                        screen = 2;
                      ...
                      return;
                    } else if (!(KycController.uploadKycFormKey.currentState
                            ?.validate() ??
                        false)) {
                      showToast("please upload images");
                    } else {
                      List<ImageUploadModel> imageModelList =
                          await imageModelFilled();
                      AllKycModel model = allKycModelFilled(
                        id: BusinessProfileData.vendorId() ?? '',
                      );
                      kycBloc.add(
                        KycUploadEvent(
                          data: model.toJson(),
              ...
              BlocListener<HiveBloc, HiveState>(
                ...
                listener: (BuildContext context, HiveState state) {
                  if (state is HiveGetBusinessKycState) {
                    KycController.businessName.text =
                        state.businessKycModel.businessRegistrationName ?? '';
                    KycController.businessDescription.text =
                        state.businessKycModel.businessDescription ?? '';
                    KycController.constitution.text =
                        state.businessKycModel.constitution ?? '';
                    KycController.gst.text =
                        state.businessKycModel.gstNumber ?? '';
                    KycController.panCard.text =
                        state.businessKycModel.panCard ?? '';
                    KycController.fssai.text =
                        state.businessKycModel.fssaiLicense ?? '';
                    KycController.vendorName.text =
                        state.businessKycModel.vendorName ?? '';
                  } else if (state is HiveGetBankKycState) {
                    KycController.bankName.text =
                        state.bankKycModel.bankName ?? '';
                    KycController.accountNumber.text =
                        state.bankKycModel.accountNumber ?? '';
                    KycController.accountName.text =
                        state.bankKycModel.accountName ?? '';
                    KycController.ifscCode.text =
                        state.bankKycModel.ifscCode ?? '';
                    KycController.checkPhoto.text =
                        state.bankKycModel.checkImageUrl ?? '';
                  } else if (state is HiveGetOwnerKycState) {
                    KycController.ownerName.text =
                        state.ownerKycModel.ownerName ?? '';
                    KycController.ownerEmailId.text =
                        state.ownerKycModel.ownerEmailId ?? '';
                    KycController.ownerContactNumber.text =
                        state.ownerKycModel.ownerContactNumber ?? '';

                    KycController.logoPhoto.text =
                        state.ownerKycModel.logoUrl ?? '';
                    KycController.businessLogoPhoto.text =
                        state.ownerKycModel.businessPhotoUrl ?? '';
                  } else if (state is HiveGetUploadKycState) {
                    KycController.businessProofPhoto.text =
                        state.uploadKycModel.businessRegistrationProof ?? '';
                    KycController.gstCertificatePhoto.text =
                        state.uploadKycModel.businessRegistrationProof ?? '';
                    KycController.panCertificatePhoto.text =
                        state.uploadKycModel.panCard ?? '';
                    KycController.fssaiCertificatePhoto.text =
                        state.uploadKycModel.fssaiLicense ?? '';
                  }
                },
                child: SizedBox(),
              ),
              ...
              BlocListener<KycBloc, KycState>(
                ...
                listener: (BuildContext context, KycState state) {
                  ...
                  } else if (state is KycUploadingProgressState) {
                    OverlayLoadingProgress.stop();
                  } else if (state is KycSuccessState ||
                      state is KycErrorState) {
                    OverlayLoadingProgress.stop();
                  }
                  ...
                  if (state is KycSuccessState) {
                    showSuccessAlert(
                      context: context,
                      title:
                          "Your business details and KYC have been submitted successfully. Profile activation is under review. Thank you!",
                      callBack: () {
                        Navigator.pop(context);
                        Navigator.pop(context);
                      },
  ...
  Future<List<ImageUploadModel>> imageModelFilled() async {
    ...
    await addImage(
      ...
      fileName: 'business_photo',
    );
    await addImage(
      listModel: imageModelList,
      controller: KycController.businessProofPhoto,
      fileName: 'business_registration_proof',
    );
    await addImage(
      listModel: imageModelList,
    ...
    await addImage(
      ...
      fileName: 'pan_card_file',
    );
    await addImage(
      listModel: imageModelList,
      controller: KycController.fssaiCertificatePhoto,
      fileName: 'fssai_license_file',
    );
  ...
  addImage({
    required List<ImageUploadModel> listModel,
    required TextEditingController controller,
    required String fileName,
  }) async {
    File? file = await loadFile(fileName: controller.text);
    if (file != null) {
      listModel.add(ImageUploadModel(file: file, fileName: fileName));
    }
  }
